package org.liusk.payment.baofu.common.http;
...
public class HttpSendModel implements Serializable {
    ...
    public String buildPostRequestForm(String formName) {
        ...
        buffer.append("<form id=\"").append(formName).append("\" name=\"").append(formName)
            .append("\" action=\"").append(url).append("\" accept-charset=\"").append(charSet)
            .append("\" method=\"" + this.getMethod().value()
                    + "\" onsubmit=\"if(isIE)document.charset='" + charSet + "'\" >\n");

        for (HttpFormParameter param : params) {
            if (param.isHidden()) {
                buffer.append("<input type=\"hidden\" name=\"").append(param.getName())
                    .append("\" value=\"").append(param.getValue()).append("\" />\n");
            } else {
                buffer.append("<input type=\"text\" name=\"").append(param.getName())
                    .append("\" value=\"").append(param.getValue()).append("\" />\n");
            }
        }
