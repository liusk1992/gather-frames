package org.liusk.payment.baofu.common.rsa;
...
import org.liusk.payment.baofu.common.util.FormatUtil;
...
public final class RsaCodingUtil {
    ...
    public static String decryptByPrivateKey(String src, PrivateKey privateKey) {
        ...
        try {
            byte[] destBytes = rsaByPrivateKey(FormatUtil.hex2Bytes(src), privateKey,
                Cipher.DECRYPT_MODE);
            if (destBytes == null) {
                return null;
            }
    ...
    public static String decryptByPublicKey(String src, PublicKey publicKey) {
        ...
        try {
            byte[] destBytes = rsaByPublicKey(FormatUtil.hex2Bytes(src), publicKey,
                Cipher.DECRYPT_MODE);
            if (destBytes == null) {
                return null;
            }
    ...
    public static byte[] rsaByPublicKey(byte[] srcData, PublicKey publicKey, int mode) {
        try {
            ...
            // 分段加密
            int blockSize = (mode == Cipher.ENCRYPT_MODE) ? RsaConst.ENCRYPT_KEYSIZE
